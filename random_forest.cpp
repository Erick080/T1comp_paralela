...
// =================================================================================
std::pair<Dataset, Dataset> train_test_split(const Dataset& full_data, double test_size = 0.2, unsigned int random_state = 42) {
    ...
    // 2. Embaralhar os índices aleatoriamente (seed fixa random_state)
...
private:
    ...
    std::pair<int, double> find_best_split(const Dataset& data, const std::vector<int>& indices, unsigned int seed) {
    ...
    std::unique_ptr<Node> build_tree(const Dataset& data, const std::vector<int>& indices, int depth, unsigned int seed) {
        ...
        auto [feature, value] = find_best_split(data, indices, seed);
        ...
        node->left = build_tree(data, left_indices, depth + 1, 2 * seed + 1);
        node->right = build_tree(data, right_indices, depth + 1, 2 * seed + 2);
...
public:
    ...
    void train(const Dataset& data, const std::vector<int>& indices, unsigned int seed) {
        root = build_tree(data, indices, 0, seed);
    }
...
public:
    RandomForest(int n_trees, int depth, int mpi_rank, int min_samples = 2)
        : num_trees(n_trees), max_depth(depth), rank(mpi_rank), min_samples_split(min_samples) {
    ...
    void train(const Dataset& data) {
        ...
        for (int i = 0; i < num_trees; ++i) {
            ...
            unsigned int seed = rank * num_trees + i;
            ...
            trees[i].train(data, sample_indices, seed);
            printf("Rank %d treinou arvore %d\n", rank, i);
	        fflush(stdout);
        }
    }
